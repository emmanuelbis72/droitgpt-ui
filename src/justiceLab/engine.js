...
// V5 ULTRA PRO — Justice Lab Engine (offline/hybride)
// Compatible JusticeLabPlay / JusticeLabAudience / Results / Dashboard
// - Audience réaliste: objections, pièces admises/écartées, audit log, scoring audience
// - Effets templates: support "effects.onAccueillir/onRejeter/onDemander"
// - Storage-friendly: patchActiveRun / activeRunId
...
export function mergeAudienceWithTemplates(caseData, apiData) {
  // apiData attendu: {turns, objections}
  const templates = Array.isArray(caseData?.objectionTemplates) ? caseData.objectionTemplates : [];

  const sceneMeta = {
    tribunal: safeStr(caseData?.meta?.tribunal || "Tribunal", 60),
    chambre: safeStr(caseData?.meta?.chambre || "Audience", 80),
    ville: safeStr(caseData?.meta?.city || caseData?.meta?.ville || "RDC", 40),
    date: nowISO().slice(0, 10),
    dossier: safeStr(caseData?.caseId || "DOSSIER", 40),
    audienceType: "Audience simulée",
    phases: [
  ...
  for (let i = 0; i < apiObs.length; i++) {
    ...
    merged.push({
      ...
      // compat: IA peut renvoyer effect/effects
  ...
  for (let j = 0; j < templates.length && merged.length < 4; j++) {
    ...
    merged.push({
      ...
      options: ["Accueillir", "Rejeter", "Demander précision"],
      // ✅ templates new format: effects (ou ancien effect)
...
export function applyAudienceDecision(run, payload) {
  ...
  // upsert (1 décision par objection)
  ...
  // recalcul score audience (heuristique globale)
...
function computeMicroScore(decision, reasoning) {
  ...
  const bonusLen = clamp(Math.floor(text.length / 160), 0, 6); // +0..6
...
function applyEffectsByDecision(run, effects, decision) {
  ...
  // Ancien format: { excludePieceIds, admitLatePieceIds, dueProcessBonus, ... }
  const d = String(decision || "").toLowerCase();
  ...
  // fallback ancien format si pas de branches
  ...
  run.state.pendingTasks = run.state.pendingTasks.slice(0, 40);
}
...
function computeAudienceScore(run) {
  ...
  // micro cumul (0..18 par décision), convertir vers 0..100
...
/* =========================================================
   SCORING LOCAL (fallback) — si tu n’appelles pas l’IA
========================================================= */
